...
const { EYES_KEY, PROJECT_NAME } = require('../commonData');
...
class EyesHelper extends Helper {
    ...
    async screenShotForVisualTest (
        appName = `${this.suiteTitle}:${this.scenarioTitle}`,
        stepName = `Step:${this.step}`
    ) {
        if (this.isNewTest) {
            this.browser = this._getBrowser();
            await this.eyes.open(this.browser, appName, PROJECT_NAME);
